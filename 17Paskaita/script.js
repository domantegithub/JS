...
//Parasykite skaiciu, kiek yra moteru tarp siu duomenu
function femaleCount(array) {
    const persons = array.filter((person) => person.gender === "Female"); //pirma atfiltruojam masyva tik su moterim
    return persons.length;
};
console.log(femaleCount(data));

//Sukurkite masyva, kuriuose būtų visų žmonių, kurie turi automibilius naujesnius nei 2000 metai.
// sukurti arrow funckija
//Isfiltruoti su nurodyta salyga
//Grazinti tik id property
//return
...
// //Sukurkite masyva zmoniu, kuriu marskineliu dydis XS arba S ir suruziuoti pagal vardus nuo A-Z
...
// //Padarykite, kad matytusi tik id, vardas ir marskineliu dydis
...
//NAMU DARBAI
//1)Funckija, kuri priims minutes ir grazins valandas

const minutesToHours=(minutes)=>`${minutes/60} hours`;
console.log(minutesToHours(30));

//2) funkcija kuri priims metus ir grazins amziu
const yearsToDecade=(years)=>`${years/10} amžius`;
console.log(yearsToDecade(23));

//3)Arrow funkcija, kuri pakels skaiciu kubu
const cube=(number)=>number*number*number;
console.log(cube(5));

//4) Arrow funkcija, kuri paskaiciuos kvadrato plota
const squareArea=(parameter1, parameter2)=>parameter1*parameter2;
console.log(squareArea(10,10));

//5) Funckija, kuri paims Stringa ir grazins pirma raide(didziaja)
const firsUpperLetter=(text)=>text.charAt(0).toUpperCase();
...
function countNamesFromA(array) {
    const persons = array.filter((person)=>person.first_name.charAt(0)==="A");
    return persons.length;
};

console.log(countNamesFromA(data));
...
console.log(countNamesK(data));

//sukurkite masyva, kur butu visu zmoniu id, kurie turi automobilius senesius nei 2000m

const idList = (array) => {
    const olderThen2000 =array.filter((person)=>person.car_year>2000);
    return olderThen2000.map((person)=>person.id);
};
console.log(idList(data));

//Sukurkite masyva, kur kuriu marskiniu dydziai didesn nei M su surusiuoti nuo Z-A
const mSizePeople = (array) => {
    const small = array.filter(
    (people) => people.shirt_size==="M"
);
const alphabet = small.sort().reverse();
return alphabet;
}

console.log(mSizePeople(data));

//Pakoreguokite, kad matytuti tik ID, lytis bei masinos marke
